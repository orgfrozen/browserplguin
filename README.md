...
- `PREPARING_SOURCE` 中的 PatchSync 网络/传输型 `RESOURCE_DOWNLOAD_FAILED` 不再立即 release Task；保持同一 Assignment/Execution 与已创建的 `export_id`，按 5s → 10s → 30s → 60s（封顶 60s）自动恢复。`409 project operation is busy` 视为临时项目锁冲突并沿用同一退避；其它明确的 4xx、身份不匹配、缺失/非法/空资源或超限仍按不可恢复错误处理。
- PatchSync `ensure-ready` 若返回显式 manual-stop `409`，BrowserPlugin 会保留同一 Assignment/Execution 并进入 `WAITING_HUMAN`，不会由自动 recovery 重试或覆盖人工维护状态。操作员执行 `make start <project>` 后，使用显式 Recover 即可在同一 Execution 上重新尝试 source preparation；Popup 会展示 manual-stop 原因与建议动作。
