...
- persist privacy-safe Live Calibration evidence locally using fixed surface ids/statuses, selector profile, page/access enums, timestamps, bounded recent runs, aggregate counts, and at most three allowlisted structural fingerprints per surface; never persist other matrix evidence/free text and never upload the ledger remotely;
...
M13 compatibility/observability (privacy-safe Popup active Task status + login/challenge fail-closed guard + selector registry versioning + privacy-safe error DOM diagnostics + local UI compatibility telemetry + read-only Live Calibration Matrix + bounded local Calibration Evidence Ledger + privacy-safe selector calibration structural fingerprints + calibration coverage gate/safe handoff report + Production Readiness Gate + Safe Validation Handoff Bundle + diagnostic screenshot safety policy v1 implemented; actual screenshot capture remains disabled/unimplemented)
```
