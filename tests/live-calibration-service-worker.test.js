...
test('service worker exposes live calibration evidence record/read/clear wiring', async () => {
  ...
  assert.match(source, /import \{ buildCalibrationCoverage \} from '\.\.\/shared\/calibration-coverage\.js';/);
  assert.match(source, /import \{ buildCalibrationCampaign \} from '\.\.\/shared\/calibration-campaign\.js';/);
  ...
  assert.match(source, /case 'GET_CALIBRATION_COVERAGE':\s*return buildCalibrationCoverage\(await calibrationEvidence\.getSummary\(\)\);/);
  assert.match(source, /case 'GET_CALIBRATION_CAMPAIGN':\s*return buildCalibrationCampaign\(await calibrationEvidence\.getSummary\(\)\);/);
