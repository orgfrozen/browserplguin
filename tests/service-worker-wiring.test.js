...
test('service worker normalizes the user workspace preference while each claimed Task keeps its captured mode', async () => {
  const source = await fs.readFile(new URL('../src/background/service-worker.js', import.meta.url), 'utf8');
  ...
  assert.match(source, /loadEffectiveSettings[\s\S]*workspaceMode:\s*normalizeWorkspaceMode\(settings\.workspaceMode\)/);
  assert.match(source, /case 'GET_SETTINGS':[\s\S]*workspaceMode:\s*normalizeWorkspaceMode/);
  assert.match(source, /case 'SAVE_SETTINGS':[\s\S]*next\.workspaceMode\s*=\s*normalizeWorkspaceMode/);
  assert.match(source, /async function setWorkspaceMode\(workspaceMode\)/);
  assert.match(source, /case 'SET_WORKSPACE_MODE':[\s\S]*setWorkspaceMode\(message\.workspaceMode\)/);
});
